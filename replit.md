...
- **Payouts**: Monthly tracking per user, status (paid/pending/cancelled/offset), yearly summaries, admin/user dashboards. Unified overview endpoints (GET /api/admin/payouts/overview/:userId/:year, GET /api/user/payouts/overview/:year) combine booking_payouts (API-calculated) and payouts (manual entries) tables. Supports multiple manual entries per month (e.g., paid + offset in same month). PDF report generation (GET /api/admin/payouts/report/:userId/:year/download) and email sending (POST /api/admin/payouts/report/:userId/:year/email) using PDFKit. Backup CSVs stored in project root (payouts_backup_*.csv, booking_payouts_backup_*.csv).
- **Agreements**: Admin-scheduled meetings, status tracking, detailed notes (public/private), notifications.
- **Beds24 Integration**: Bidirectional sync for bookings and blocks, delta sync, optimized sync window (-30 to +360 days), 5-minute sync frequency, protection for CSV-imported events, automatic email removal from event descriptions. Multi-property sync via user_properties table (Beds24ApiClient accepts overridePropId). Availability-based blackout detection via inventory/availability endpoint with merged date range filtering (combines overlapping/adjacent booking and owner block ranges before coverage checks). Property names shown in blackout event titles for multi-property users. Local owner blocks sync to Beds24 as "black" status bookings with roomId resolution. Owner block echo prevention: skips incoming Beds24 bookings with firstName="Eiersperre tid" and status="black", and filters availability blackouts covered by local red owner blocks or skipped owner block date ranges. Admin bulk sync endpoint at POST /api/admin/beds24-sync-local-blocks. Admin cleanup endpoint at POST /api/admin/beds24-delete-bookings. Beds24 API v2 response format: array of objects with `new.id` for booking IDs.
- **Google Calendar Integration**: Users can connect Google Calendar via secret iCal URL. Events sync automatically every minute as red owner blocks (#ef4444) with "[Importert fra Google Kalender]" description tag. Settings page has dedicated "Google Kalender" tab with instructions, feed management (enable/disable/delete), and status display. Uses `isGoogleCalendar` flag on `ical_feeds` table. Backend endpoint: POST /api/ical-feeds/google-calendar.
